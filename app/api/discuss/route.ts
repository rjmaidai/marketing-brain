...
function safeParseSelection(raw: string): HeadSelection | null {
      try {
              ...
              type HEntry = { id: string; reason?: string };
              const heads = (parsed.heads as HEntry[])
          ...
          async start(controller) {
            ...
            try {
                        if (body.idea) {
                          ...
                          const enrichedHeads: HeadRow[] = selection.heads
                                        .map((h) => {
                      ...
                      if (body.followUp) {
                          ...
                          type HeadObj = { id: string; name: string; segment: string; color: string };
                                    const followUpHeads: HeadObj[] = followUpHeadIds
                                      .map((id) => getHead(id))
                      ...
                      send("error", {
                                    message: "Ungültige Anfrage: idea oder followUp fehlt.",
                      });
                        controller.close();
            } catch (err) {
                        const message =
